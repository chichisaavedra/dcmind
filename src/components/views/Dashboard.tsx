 ...
 * Permite subir archivos mediante "Drag & Drop", ver la actividad reciente,
 * visualizar las métricas en widgets superiores, y navegar en la jerarquía 
 * de carpetas principales y subcarpetas (Biblioteca Visual).
 * 
 * @param onNavigate - Función para cambiar la vista actual del layout principal 
 * @param files - Lista de documentos del usuario
 * @param folders - Objeto que mapea las carpetas a sus subcarpetas
 * @param userPlan - Plan actual del usuario para mostrar/ocultar funciones (ej. límite de archivos)
 * @param userName - Nombre del usuario autenticado
 * @param searchQuery - Término de búsqueda global para filtrar inmediatamente los archivos
 */
export default function Dashboard({
  ...
  // -- Navegación visual de carpetas en pantalla central --
  ...
  // -- Estados del Proceso de 'Smart Upload' (Subida Inteligente IA) --
  /**
   * Ciclo de vida de subida inteligente:
   * 'idle' -> 'analyzing' (simula lectura de PDF) -> 'suggestions' (propone donde rutear) -> 'routing' (guarda)
   */
  ...
  useEffect(() => {
    const handleOpenFile = (e: any) => setSelectedFileId(e.detail.id);
  ...
  const handleDrop = (e: DragEvent) => {
    ...
    if (files && files.length > 0) initiateSmartUpload(Array.from(files));
  };
  ...
  const initiateSmartUpload = async (incomingFiles: File[]) => {
    if (setRawFiles) {
      setRawFiles(prev => [...prev, ...incomingFiles]);
    }

    setSmartUploadState('analyzing');

    try {
      // Process files genuinely via Gemini API instead of using timeouts and mock data
      const processedFiles = [];

      for (const file of incomingFiles) {
        // Ask Gemini to summarize and categorize the file
        let aiResult = { summary: 'Documento procesado por IA de DocMind.', folder: 'Sin Clasificar', subfolder: 'General' };

        try {
          const { askGemini } = await import('../../lib/gemini');
          // Improved prompt to separate Category/Folder from Subfolder/Type
          const prompt = `Analiza este archivo "${file.name}" y responde en formato JSON puro:
{
  "summary": "Resumen de 1 oración",
  "folder": "Categoría principal (ej: Finanzas, Legal, Personal, Trabajo)",
  "subfolder": "Subcategoría específica (ej: Facturas, Contratos, Identidad, Proyectos)"
}
Evita bloques de código, solo el objeto JSON.`;

          const rawResponse = await askGemini(prompt, [file]);
          ...
          if (jsonMatch) {
            try {
              ...
              aiResult = {
                ...
                folder: parsed.folder || aiResult.folder,
                subfolder: parsed.subfolder || parsed.folder || 'General'
              };
            } catch (pErr) {
              console.warn("JSON parse error:", pErr);
            }
          }
        } catch (apiError) {
          console.error("Gemini no pudo clasificar un archivo:", apiError);
        }
        ...
        processedFiles.push({
          ...
          name: file.name,
          type: extension,
          tagType: aiResult.subfolder, // This will be the badge
          suggestionFolder: aiResult.folder,
          suggestionSubfolder: aiResult.subfolder,
          folder: 'Personal',
          subfolder: aiResult.subfolder,
          size: file.size,
      ...
      console.error("Fallo general en la subida inteligente", e);
  ...
  const confirmSmartUpload = () => {
    ...
    if (setFiles && uploadedFiles.length > 0) {
      if (setFolders) {
        setFolders((prev: Record<string, string[]>) => {
          const updated = { ...prev };
          uploadedFiles.forEach(f => {
            const fName = f.suggestionFolder;
            const subName = f.suggestionSubfolder;

            if (fName && typeof fName === 'string') {
              if (!updated[fName]) {
                updated[fName] = [];
              }
              // Add subfolder to folder if not present
              if (subName && !updated[fName].includes(subName)) {
                updated[fName] = [...updated[fName], subName];
              }
            }
          });
          return updated;
      ...
      const finalFiles = uploadedFiles.map(({ suggestionFolder, suggestionSubfolder, ...f }) => ({
        ...f,
        folder: suggestionFolder,
        subfolder: suggestionSubfolder
      }));
      setFiles(prev => [...finalFiles, ...prev]);
    }
  ...
  return (
    <>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-[3rem] border border-gray-200/50 p-8 shadow-sm space-y-8">
              ...
              <div className="space-y-4">
                ...
                <button onClick={() => alert('La revisión de sugerencias IA requiere conexión con el motor de IA del backend. (TODO)')} className="w-full py-4 bg-gray-900 text-white rounded-2xl font-bold text-sm hover:bg-black transition-all">
                  Revisar Ahora
                </button>
              </div>
            </div>
      ...
      {selectedFile && createPortal(
        <div className="fixed top-0 left-0 right-0 bottom-0 z-[9999] flex items-center justify-center p-4 sm:p-6 outline-none overflow-hidden">
          <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-md animate-fade-in" onClick={() => setSelectedFileId(null)} />
          <div className="relative w-full max-w-3xl bg-white rounded-[3rem] shadow-2xl overflow-hidden animate-slide-up border border-white max-h-[90vh] flex flex-col z-[10000]">
            <div className="p-6 sm:p-8 pb-0 flex justify-between items-start">
              <div className="flex items-center gap-5">
                <div className={cn(
                  "w-16 h-16 rounded-3xl flex items-center justify-center shadow-md",
                  selectedFile.type === 'pdf' ? 'bg-red-50 text-red-500' : 'bg-blue-50 text-blue-500'
                )}>
                  <FileText size={28} />
                </div>
                <div>
                  <h3 className="text-2xl font-black text-gray-900 tracking-tight leading-tight">{selectedFile.name}</h3>
                  <div className="flex items-center gap-3 mt-1.5">
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{selectedFile.folder}</span>
                    <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full" />
                    <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Analizado</span>
                  </div>
                </div>
              </div>
              <button onClick={() => setSelectedFileId(null)} className="p-2 hover:bg-gray-100 rounded-full text-gray-400 transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-6 sm:p-8 flex-1 overflow-y-auto custom-scrollbar space-y-8">
              <div className="space-y-3">
                <div className="flex items-center gap-2 opacity-50">
                  <Sparkles size={14} />
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Resumen de Inteligencia</span>
                </div>
                <div className="bg-[#F5F5F7] p-6 rounded-[2rem] relative overflow-hidden group">
                  <div className="absolute top-0 right-0 w-24 h-24 bg-indigo-500/5 rounded-full -mr-12 -mt-12 blur-3xl" />
                  <p className="text-base text-gray-800 leading-relaxed font-medium">{selectedFile.summary}</p>
                </div>
              </div>
            ...
            <div className="p-6 sm:p-8 pt-0 flex gap-4">
              <button className="flex-1 py-4 bg-gray-900 text-white rounded-[1.5rem] font-bold hover:bg-black transition-all flex items-center justify-center gap-3 active:scale-[0.99] shadow-lg shadow-gray-200">
                <Download size={20} />
                Visualizar Documento
              </button>
              <div className="flex gap-3">
                <button className="w-14 h-14 bg-white border border-gray-200 rounded-[1.2rem] flex items-center justify-center text-gray-600 hover:bg-gray-50 transition-all">
                  <Share2 size={24} />
                </button>
                <button className="w-14 h-14 bg-white border border-gray-200 rounded-[1.2rem] flex items-center justify-center text-red-500 hover:bg-red-50 transition-all">
                  <Trash2 size={24} />
                </button>
              </div>
            </div>
            ...
            {smartUploadState === 'suggestions' && (
              ...
              >
                <div className="p-10 pb-6 border-b border-gray-100 flex justify-between items-center bg-white z-20">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-500 mt-1 uppercase tracking-widest leading-relaxed">Confirma o elimina sugerencias antes de procesar.</p>
                    </div>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-10 bg-[#F5F5F7] space-y-8">
                  {[...new Set(uploadedFiles.map(f => f.suggestionFolder))].map(folderGroup => (
                    <div key={folderGroup} className="space-y-4">
                      <div className="flex items-center gap-3">
                        <Folder size={20} className="text-indigo-500" />
                        <h4 className="text-lg font-extrabold text-gray-900">
                          Carpeta sugerida: <span className="text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg border border-indigo-100">{folderGroup}</span>
                        </h4>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        {uploadedFiles.filter(f => f.suggestionFolder === folderGroup).map(file => (
                          <div key={file.id} className="bg-white p-5 rounded-[1.5rem] border border-gray-200 shadow-sm flex items-start justify-between gap-4 hover:border-indigo-300 transition-all group/item">
                            <div className="flex items-start gap-4 min-w-0">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-[10px] text-gray-400 font-medium line-clamp-1 mt-0.5">{file.summary}</p>
                              </div>
                            </div>
                            <button
                              onClick={() => setUploadedFiles(prev => prev.filter(f => f.id !== file.id))}
                              className="p-2 hover:bg-red-50 text-gray-300 hover:text-red-600 rounded-xl transition-all group-hover/item:text-red-400"
                              title="Omitir este archivo"
                            >
